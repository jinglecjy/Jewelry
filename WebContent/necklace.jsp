...
<% 
	...
	Statement stmt=con.createStatement();
	String sql=String.format("select * from product where pclass='necklace' limit %d,%d;", pgno*pgcnt,pgcnt);
...
<body>
	...
	<div id="screen">
		<div class="tableShow">
			...
			<%=SearchMsg %> <br><br><br>
			<div class="turnPage1">
				<a href="necklace.jsp?pgno=<%=pgprev%>&pgcnt=<%=pgcnt%>&judge=<%=judge%>">上一页</a>
			</div> 
			<div class="turnPage2">
				<a href="necklace.jsp?pgno=<%=pgnext%>&pgcnt=<%=pgcnt%>&judge=<%=judge%>">下一页</a>
			</div>
		</div>
	</div>
	</div>
