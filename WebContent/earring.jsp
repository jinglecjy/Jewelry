...
<% 
	...
	Statement stmt=con.createStatement();
	String sql=String.format("select * from product where pclass='earring' limit %d,%d;", pgno*pgcnt,pgcnt);
...
<body>
	...
	<div id="screen">
		<div class="tableShow">
			...
			<%=SearchMsg %> <br><br><br>
			<div class="turnPage1">
				<a href="earring.jsp?pgno=<%=pgprev%>&pgcnt=<%=pgcnt%>&judge=<%=judge%>">上一页</a>
			</div> 
			<div class="turnPage2">
				<a href="earring.jsp?pgno=<%=pgnext%>&pgcnt=<%=pgcnt%>&judge=<%=judge%>">下一页</a>
			</div>
		</div>
	</div>
	</div>
